<template>
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="500px" @close="close">
    <el-form ref="form" label-width="80px" :model="form" :rules="rules">
      <el-form-item label="代號" prop="type_id">
        ...
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            if (!this.form_lock) {
              console.log('新增')
              this.title = '新增'
              this.url = 'http://localhost:5252/api/translation_team_type'
              ...
              console.log('編輯')
              this.title = '編輯'
              this.url = `http://localhost:5252/api/translation_team_type/${this.form.type_id}`
              this.params = this.form
