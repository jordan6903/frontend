<template>
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="500px" @close="close">
    <el-form ref="form" label-width="80px" :model="form" :rules="rules">
      <el-form-item label="代碼" prop="staff_typeid">
        <el-input v-model.number="form.staff_typeid" autocomplete="off" :disabled="form_lock" maxlength="3" type="number" />
      </el-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        form_lock: false,
        rules: {
          staff_typeid: [{ required: true, trigger: 'blur', message: '請輸入代號' }],
    ...
    methods: {
      ...
      toUpperCase(event) {
        this.form.staff_typeid = event.toUpperCase()
      },
      save() {
        console.log('===save')
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            if (!this.form_lock) {
              console.log('新增')
              this.title = '新增'
              this.url = 'http://localhost:5252/api/staff_type'
              ...
              console.log('編輯')
              this.title = '編輯'
              this.url = `http://localhost:5252/api/staff_type/${this.form.staff_typeid}`
              this.params = this.form
