<template>
  <div class="table-container">
    ...
    <el-table
      ...
      :data="list"
      :default-sort="{ prop: 'staff_typeid' }"
    ...
    >
      ...
      <el-table-column label="代碼" prop="staff_typeid" show-overflow-tooltip sortable width="95" />
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        url: 'http://localhost:5252/api/staff_type',
    ...
    methods: {
      ...
      handleDelete(row) {
        ...
        console.log(row.staff_typeid)
        if (row.staff_typeid) {
          this.$baseConfirm('你確定要刪除當前項嗎?', null, async () => {
            let ls_url = this.url
            ls_url += `/${row.staff_typeid}`
