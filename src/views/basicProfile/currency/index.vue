<template>
  <div class="table-container">
    ...
    >
      ...
      <el-table-column label="代碼" prop="currency_id" show-overflow-tooltip sortable width="95" />
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        url: 'http://localhost:5252/api/currency',
        return_msg: '',
        return_success: '',
        imgShow: true,
        list: [],
    ...
    methods: {
      ...
      handleDelete(row) {
        ...
        console.log(row.currency_id)
        if (row.currency_id) {
          this.$baseConfirm('你確定要刪除當前項嗎?', null, async () => {
            //const { msg } = await doDelete({ ids: row.id })

            let ls_url = this.url
            ls_url += `/${row.currency_id}`
