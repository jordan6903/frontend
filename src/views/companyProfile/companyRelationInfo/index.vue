<template>
  <div class="table-container">
    ...
    >
      ...
      <el-table-column label="代碼" prop="relation_id" show-overflow-tooltip sortable width="95" />
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        url: 'http://localhost:5252/api/company_relation_info',
        return_msg: '',
        return_success: '',
        imgShow: true,
        list: [],
    ...
    methods: {
      ...
      handleDelete(row) {
        ...
        console.log(row.relation_id)
        if (row.relation_id) {
          this.$baseConfirm('你確定要刪除當前項嗎?', null, async () => {
            let ls_url = this.url
            ls_url += `/${row.relation_id}`
