<template>
  <div class="table-container">
    ...
    <el-table
      ...
      :data="list"
      :default-sort="{ prop: 'p_type_class' }"
    ...
    >
      ...
      <el-table-column label="代碼" prop="p_type_class" show-overflow-tooltip sortable width="95" />
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        url: 'http://localhost:5252/api/product_type_class',
        return_msg: '',
        return_success: '',
        imgShow: true,
        list: [],
    ...
    methods: {
      ...
      handleDelete(row) {
        ...
        console.log(row.p_type_class)
        if (row.p_type_class) {
          this.$baseConfirm('你確定要刪除當前項嗎?', null, async () => {
            let ls_url = this.url
            ls_url += `/${row.p_type_class}`
