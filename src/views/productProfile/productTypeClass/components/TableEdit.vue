<template>
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="500px" @close="close">
    <el-form ref="form" label-width="80px" :model="form" :rules="rules">
      <el-form-item label="代碼" prop="p_type_class">
        <el-input v-model.number="form.p_type_class" autocomplete="off" :disabled="form_lock" maxlength="3" type="number" />
      </el-form-item>
      <el-form-item label="名稱" prop="name">
        <el-input v-model.trim="form.name" autocomplete="off" maxlength="20" />
      </el-form-item>
      <el-form-item label="敘述" prop="content">
        <!--<el-input v-model.trim="form.content" autocomplete="off" maxlength="100" />-->
        <el-input v-model="form.content" autocomplete="off" maxlength="1000" type="textarea" />
      </el-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        form_lock: false,
        rules: {
          p_type_class: [{ required: true, trigger: 'blur', message: '請輸入代號' }],
    ...
    methods: {
      ...
      toUpperCase(event) {
        this.form.p_type_class = event.toUpperCase()
      },
      save() {
        console.log('===save')
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            if (!this.form_lock) {
              console.log('新增')
              this.title = '新增'
              this.url = 'http://localhost:5252/api/product_type_class'
              ...
              console.log('編輯')
              this.title = '編輯'
              this.url = `http://localhost:5252/api/product_type_class/${this.form.p_type_class}`
              this.params = this.form
