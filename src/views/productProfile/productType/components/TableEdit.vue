...
-->
      <el-form-item label="屬性/標籤分類" prop="p_type_id">
        <el-select v-model="form.p_type_id" placeholder="請選擇分類">
          <el-option
    ...
    data() {
      return {
        ...
        rules: {
          ...
          p_type_id: [{ required: true, trigger: 'blur', message: '請選擇屬性/標籤' }],
        },
        title: '',
        ...
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            if (!this.form_lock) {
              console.log('新增')
              this.title = '新增'
              this.url = 'http://localhost:5252/api/product_type'
              ...
              console.log('編輯')
              this.title = '編輯'
              this.url = `http://localhost:5252/api/product_type/${this.form.id}`
              this.params = this.form
