<template>
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="1200px" @close="close">
    <el-button type="primary" @click="export_file">輸出本體</el-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        url5: 'http://localhost:5252/api/export_view/get1_1',
        url6: 'http://localhost:5252/api/export_view/get1_2',
    ...
    methods: {
      ...
      async export_file() {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          title += `<hr><a id="${data[i]['c_id']}_h" href="${ls_url}" target="_blank"><h1>`
          title += data[i]['c_Name']
          ...
          title += `<a class="b_pic" onclick="pic_vis('${data[i]['c_id']}');this.style.display='none'"><div id="show_com_pic"><font size="2">展開細節</font></div></a><br>\n`

          if (data[i]['c_Intro'] != '') {
            title += `<p class='${data[i]['c_id']}' style="display: none;">`
          ...
          for (let j = 0; j < series.length; j++) {
            if (series[j]['add_word_Use_yn']) {
              content += `<input type="hidden" class="${data[i]['c_id']}_p" value="`
              content += `${series[j]['add_word']}`
              content += `"><p class="${data[i]['c_id']}_p"></p>`
            }
      ...
      async export_file2() {
        ...
        //整理
        for (let i = 0; i < data.length; i++) {
          ls_tmp += `<a href="#${data[i]['c_id']}_h"><font size="1">${data[i]['c_Name']}</font></a><br>\n`
      ...
      async export_file3() {
        ...
        let fileName = 'BloggerJson檔.json'
        /*        let content = ''
      ...
      async generateData2() {
        ...
        for (let i = 0; i < this.clist2.length; i++) {
          for (let j = 0; j < this.plist2.length; j++) {
            if (this.clist2[i]['esp_id'] == this.plist2[j]['esp_id']) {
