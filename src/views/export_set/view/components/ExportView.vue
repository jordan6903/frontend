<template>
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="1200px" @close="close">
    ...
    <table id="table1" class="export">
      ...
      <tr v-for="data in clist" :key="data.esp_id">
        <td></td>
        <td>{{ data.c_Name }}</td>
        <td>{{ data.p_Name }}</td>
        <td>{{ data.p_CName }}</td>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        export_batch: 0,
        url: 'http://localhost:5252/api/export_view/get1_1',
        url2: 'http://localhost:5252/api/export_view/get1_2',
      ...
      async generateData() {
        ...
        for (let i = 0; i < this.clist.length; i++) {
          for (let j = 0; j < this.plist.length; j++) {
            if (this.clist[i]['esp_id'] == this.plist[j]['esp_id']) {
