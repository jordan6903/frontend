    ...
    <div class="div_out">
      ...
      <div class="div_mid">
        ...
        <div class="div_search">
          <input v-model="search" autocomplete="off" class="input_search" placeholder="請輸入代號或公司名稱" type="text" />
          <span class="input_search_icon"><i class="el-input__icon el-icon-search"></i></span>
        </div>
        <ul class="ul_item_group_drag">
          <li v-for="company in filterList" :key="company.id" class="li_item">
            <input v-model="select_item" class="checkbox" name="list" type="checkbox" :value="company.id" />
            <label :for="company.id">
              {{ company.sort }} - {{ company.c_Name }} ( 子分類 {{ company.snumber }}，遊戲 {{ company.pnumber }} )
            </label>
          </li>
        </ul>
      </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        url3: 'http://localhost:5252/api/export_set_product',
    ...
    computed: {
      filterList() {
        let DataArry = this.list.filter(
          (item) =>
            item.c_Name.toLowerCase().includes(this.search.toLowerCase()) || item.c_id.toLowerCase().includes(this.search.toLowerCase())
        )
        return DataArry
      },
    },
    ...
    methods: {
      ...
      async generateData() {
        ...
        for (let i = 0; i < this.slist.length; i++) {
          for (let j = 0; j < this.plist.length; j++) {
            if (this.slist[i]['esps_Id'] == this.plist[j]['esps_id']) {
        ...
        for (let i = 0; i < this.list.length; i++) {
          for (let j = 0; j < this.slist.length; j++) {
            if (this.list[i]['id'] == this.slist[j]['esc_id']) {
      ...
      selectAll() {
        ...
        for (let i = 0; i < this.filterList.length; i++) {
          this.select_item.push(this.filterList[i]['id'])
        }
      },
      selectClear() {
      ...
      async save() {
        ...
        for (let i = 0; i < this.select_item.length; i++) {
          for (let j = 0; j < this.list.length; j++) {
            if (this.select_item[i] == this.list[j]['id']) {
              this.insert_company.push({
                export_batch: this.batch2,
                c_id: this.list[j]['c_id'],
        ...
        console.log(this.insert_company)

        let ls_url = `${this.url}/several`

        await axios
          .post(ls_url, this.insert_company)
        ...
        let ls_companyid = this.return_data.ids //回傳的esc_id
        let ls_companycid = this.return_data.c_id //回傳的c_id

        console.log(ls_companyid)
        console.log(ls_companycid)

        let ls_url2 = `${this.url2}`

        //series跟product
        for (let i = 0; i < ls_companycid.length; i++) {
          for (let j = 0; j < this.list.length; j++) {
            if (ls_companycid[i] == this.list[j]['c_id']) {
              let series_data = this.list[j]['series_data']

              for (let k = 0; k < series_data.length; k++) {
                let newarray = {
                  ESC_id: ls_companyid[i],
                ...
                if (this.return_success == 'N') {
                  ...
                  //product
                  this.insert_product = []
                  let ls_seriesid = this.return_data.id //回傳的esps_id
