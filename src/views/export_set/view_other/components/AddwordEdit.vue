<template>
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="500px" @close="close">
    <el-form ref="form" label-width="80px" :model="form" :rules="rules">
      <el-form-item label="代碼" prop="esos_Id">
        <el-input v-model.number="form.esos_Id" autocomplete="off" :disabled="true" maxlength="6" type="number" />
      </el-form-item>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        url: 'http://localhost:5252/api/export_set_other_series',
        esos_Id: 0,
        params: '',
        return_msg: '',
        return_success: '',
        ...
        form_lock: false,
        rules: {
          esos_Id: [{ required: true, trigger: 'blur', message: '請輸入代號' }],
        },
        title: '',
    ...
    methods: {
      async showEdit(row) {
        ...
        if (!row) {
          ...
          this.$baseMessage('查無esos_Id', 'success')
        } else {
          this.title = '編輯html'
          ...
          let ls_url = `${this.url}/getbyid/${this.esos_Id}`
      ...
      save() {
        ...
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            console.log('編輯')
            this.title = '編輯'
            let ls_url = `${this.url}/putaddword/${this.esos_Id}`
